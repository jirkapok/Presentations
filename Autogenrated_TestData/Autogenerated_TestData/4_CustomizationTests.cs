...
using Task = Autogenerated_TestData.DataStructures.Task;
...
public class FixedStringLengthCustomization(int maxLength) : ICustomization
{
    ...
    {
        fixture.Customize<string>(composer => composer.FromFactory(() => CreateFixLengthString(maxLength)));
    }

    private static string CreateFixLengthString(int maxLength) => Guid.NewGuid().ToString().Substring(0, maxLength);
}